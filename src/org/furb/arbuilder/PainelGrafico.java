...
public class PainelGrafico {
    ...
    public PainelGrafico(Controle controle, JFrame parent, JPanel painelGraficoPai, JList listaTabela) {
    ...
    public final void criarNovoPainelGrafico(JPanel painelGraficoPai) {
        ...
        painelGrafico.setSize(new Dimension(painelGraficoPai.getSize().width - 40, painelGraficoPai.getSize().height - 40));
    ...
    public final void inserirTabelaNoGrafico(String nomeTabela, int x, int y) 
    {
        ...
        GraphConstants.setBounds(novaTabela.getAttributes(), new Rectangle2D.Double(x, y, nomeTabela.length() * 10 + 10, 30));
        GraphConstants.setOpaque(novaTabela.getAttributes(), true);
        GraphConstants.setBorder(novaTabela.getAttributes(), BorderFactory.createLineBorder(new Color(0, 102, 153), 2));
    ...
    public void inserirLigacaoNoGrafico(int x1, int y1, int x2, int y2) {
        ...
        if(this.primeiraCelula == null) { // Achou primeira celula
            Object[] cells = DefaultGraphModel.getAll((DefaultGraphModel)this.painelGrafico.getModel());
            
            this.painelGrafico.setSelectionCells(cells);
            
            this.primeiraCelula = (DefaultGraphCell)this.painelGrafico.getSelectionCellAt(new Point(x1, y1));
            if(this.primeiraCelula == null) {
                return;
            }
            if(this.primeiraCelula.getUserObject() instanceof Tabela) {
                JOptionPane.showMessageDialog(null, "Para realizar um relacao escolha primeiro um operador!", "Erro!", JOptionPane.ERROR_MESSAGE);
                this.limparAtributo();
                return;
            }
            if(x2 < 0 && y2 < 0) {
        ...
        // Acha segunda celula
        if(x2 < 0 && y2 < 0) {
            segundaCelula = (DefaultGraphCell)this.painelGrafico.getSelectionCellAt(new Point(x1, y1));
        }
        else {
            segundaCelula = (DefaultGraphCell)this.painelGrafico.getSelectionCellAt(new Point(x2, y2));
        }

        if(segundaCelula == null) { // Se nao existir ainda segunda celula cai fora
            return;
        }
        ...
        if(v1 == v2) {
            JOptionPane.showMessageDialog(null, "Nao e possivel realizar uma relacao para o mesmo operador!", "Erro!", JOptionPane.ERROR_MESSAGE);
            this.limparAtributo();
            return;
        }
        
        if(primeiraCelula.getUserObject() instanceof Ordenacao && this.controle.getEstruturaDigrafo().getAdjacencias((Vertice)primeiraCelula.getUserObject()).size() > 0) {
            this.limparAtributo();
            return;
        }
        if(primeiraCelula.getUserObject() instanceof SelecaoProjecao && this.controle.getEstruturaDigrafo().getAdjacencias((Vertice)primeiraCelula.getUserObject()).size() > 0) {
            this.limparAtributo();
            return;
        }
        if(primeiraCelula.getUserObject() instanceof UniaoProdutoCartesiano && this.controle.getEstruturaDigrafo().getAdjacencias((Vertice)primeiraCelula.getUserObject()).size() > 1) {
            this.limparAtributo();
            return;
        }
        if(primeiraCelula.getUserObject() instanceof Diferenca && this.controle.getEstruturaDigrafo().getAdjacencias((Vertice)primeiraCelula.getUserObject()).size() > 1) {
            this.limparAtributo();
            return;
        }
        if(primeiraCelula.getUserObject() instanceof Agrupamento && this.controle.getEstruturaDigrafo().getAdjacencias((Vertice)primeiraCelula.getUserObject()).size() > 0) {
            this.limparAtributo();
            return;
        }
        if(primeiraCelula.getUserObject() instanceof JuncaoExternaEsquerda && this.controle.getEstruturaDigrafo().getAdjacencias((Vertice)primeiraCelula.getUserObject()).size() > 1) {
            this.limparAtributo();
            return;
        }
        ...
        ligacao.setUserObject(this.controle.getEstruturaDigrafo().getArestaEntreVertices(v1, v2));
    ...
    public void inserirOperadorNoGrafico(String parametro1, String parametro2, int x, int y) {
        ...
        if(parametro1 == null && parametro2 == null) {
            ...
            if(this.isAgrupamento) {
                ...
                a1 = JOptionPane.showInputDialog("Digite a(s) coluna(s) agrupadoras:");
                if(a1 == null) {
                    this.limparAtributo();
                    return;
                }
                a2 = JOptionPane.showInputDialog("Digite a(s) coluna(s) projetadas:");
                if(a2 == null) {
                    this.limparAtributo();
                    return;
                }
                if(a1.isEmpty() && a2.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Nao e possivel adicionar operador sem parametro(s)!", "Erro!", JOptionPane.ERROR_MESSAGE);
                    this.limparAtributo();
                    return;
                }
            }
                ...
                if(this.exigeParametro) {
                    ...
                    args = JOptionPane.showInputDialog("Digite o(s) parametro(s) para o operador " + this.operador.toString() + ":");
                    if(args == null) {
                        this.limparAtributo();
                        return;
                    }
                    if(args.isEmpty()) {
                        JOptionPane.showMessageDialog(null, "Nao e possível adicionar operador sem parametro(s)!", "Erro!", JOptionPane.ERROR_MESSAGE);
                        this.limparAtributo();
                        return;
                    }
                }
        ...
        if(args != null && (this.operador.getOperador().getClass() == Selecao.class || this.operador.getOperador().getClass() == Projecao.class)) {
            ((SelecaoProjecao)this.operador.getOperador()).setParametro(args);
            ((SelecaoProjecao)this.operador.getOperador()).setEntrada(new Vertice(""));
        }
        
        if( args != null && this.operador.getOperador().getClass() == JuncaoExternaEsquerda.class ) {
            ((JuncaoExternaEsquerda)this.operador.getOperador()).setParametro(args);
            ((JuncaoExternaEsquerda)this.operador.getOperador()).setEntrada(new Vertice(""));
        }
        
        if(args != null && (this.operador.getOperador().getClass() == Ordenacao.class )) {
            ((Ordenacao)this.operador.getOperador()).setParametro(args);
            ((Ordenacao)this.operador.getOperador()).setEntrada(new Vertice(""));
        }
        
        if(this.operador.getOperador().getClass() == Agrupamento.class) {
            if(a1 != null) {
                ((Agrupamento)this.operador.getOperador()).setColunasAgrupadoras(a1);
            }
            if(a2 != null) {
                ((Agrupamento)this.operador.getOperador()).setColunasProjetadas(a2);
            }
        }
        ...
        GraphConstants.setLineStyle(novoOperador.getAttributes(), GraphConstants.STYLE_ORTHOGONAL);
        GraphConstants.setBounds(novoOperador.getAttributes(), new Rectangle2D.Double(x, y, this.operador.getOperador().toString().length() * 10 + 10, 30));
        GraphConstants.setOpaque(novoOperador.getAttributes(), true);
        GraphConstants.setBorder(novoOperador.getAttributes(), BorderFactory.createLineBorder(new Color(100, 100, 100), 1));
        GraphConstants.setEditable(novoOperador.getAttributes(), false);
        GraphConstants.setIcon(novoOperador.getAttributes(), new javax.swing.ImageIcon(getClass().getResource( Operadores.getImageForOparador( v.toString() ))));
    ...
    private Vertice buildVertice( String args , String a1 , String a2 ) {
        ...
        if(this.operador.getOperador() instanceof JuncaoExternaEsquerda ) {
        	v = new JuncaoExternaEsquerda(this.operador.getOperador().getNome(), args);
        }
    ...
    {     
        ...
        for(int i = 0; i < listaRemover.length; i++) {
            if(((DefaultGraphCell)listaRemover[i]).getUserObject() instanceof Vertice) {
                this.controle.getEstruturaDigrafo().removeVertice((Vertice)((DefaultGraphCell)listaRemover[i]).getUserObject());
                this.controle.getEstruturaDigrafo().removeArestas((Vertice)((DefaultGraphCell)listaRemover[i]).getUserObject());
            }
            else if(((DefaultGraphCell)listaRemover[i]).getUserObject() instanceof Aresta) {
                this.controle.getEstruturaDigrafo().removeAresta((Aresta)((DefaultGraphCell)listaRemover[i]).getUserObject());
            }
        }
        this.painelGrafico.getGraphLayoutCache().remove(this.painelGrafico.getSelectionCells(), false, true);
    }
    ...
    public void manipularCliqueDoMouse(MouseEvent evt) 
    {
        ...
        try {
        	...
        	cell = (DefaultGraphCell)this.painelGrafico.getSelectionCellAt(new Point(evt.getX(), evt.getY()));
        	
            // Limpa atributo operador e ligacao caso botao pressionado seja o da direita
            ...
            if( evt.getButton() == MouseEvent.BUTTON1 ) {
                ...
                {
                    Object celula = ((DefaultGraphCell)this.painelGrafico.getSelectionCell()).getUserObject();
                    
                    if(celula instanceof Tabela) {
                        ((Interface)this.parent).setEditarParametrosTabela(((Vertice)celula).getNome());
                    }
                    
                    else if(celula instanceof SelecaoProjecao) {
                        ((Interface)this.parent).setEditarParametrosOperador(((SelecaoProjecao)celula).getNome(), ((SelecaoProjecao)celula).getParametro(), "", false);
                    }

                    else if(celula instanceof JuncaoExternaEsquerda ) {
                        ((Interface)this.parent).setEditarParametrosOperador(((JuncaoExternaEsquerda)celula).getNome(), ((JuncaoExternaEsquerda)celula).getParametro(), "", false);
                    }
                    
                    else if(celula instanceof Ordenacao) {
                        ((Interface)this.parent).setEditarParametrosOperador(((Ordenacao)celula).getNome(), ((Ordenacao)celula).getParametro(), "", false);
                    }
                    
                    else if(celula instanceof Agrupamento) {
                        ((Interface)this.parent).setEditarParametrosOperador(((Agrupamento)celula).getNome(), ((Agrupamento)celula).getColunasAgrupadoras(), ((Agrupamento)celula).getColunasProjetadas(), true);
                    }
                    else {
                        ((Interface)this.parent).setEditarParametrosTabela(((Operador)celula).getNome());
                    }
                }
                else {
        ...
        if(this.operador != null || this.ligacao != null) {
            if(this.operador != null) { // Caso tenha de inserir GraficoLigacao
                this.inserirOperadorNoGrafico(null, null, evt.getX(), evt.getY());
            }
            else if(this.ligacao != null) {
                this.inserirLigacaoNoGrafico(evt.getX(), evt.getY(), -1, -1); // Senao insere GraficoOperador
            }
        }
        else if(this.listaTabela.getSelectedValue() != null) {
            // Insere componente no grafico
            this.inserirTabelaNoGrafico(listaTabela.getSelectedValue().toString(), evt.getX(), evt.getY());

            // E limpa selecao da lista para nao duplicar tabelas no grafico quando clicar
    ...
    public final void setNovoOperador(int tipoOperador) {
        ...
        switch( tipoOperador ) 
        {
            case 0: v = new Selecao( Operadores.SELECAO.getOperador() );							this.exigeParametro = true;  	break;
            case 1: v = new Projecao( Operadores.PROJECAO.getOperador() );							this.exigeParametro = true;  	break;
            case 2: v = new Uniao( Operadores.UNIAO.getOperador() );								this.exigeParametro = false; 	break;
            case 3: v = new ProdutoCartesiano( Operadores.PRODUTO_CARTESIANO.getOperador() );		this.exigeParametro = false; 	break;
            case 4: v = new Agrupamento( Operadores.AGRUPAMENTO.getOperador() );					this.isAgrupamento  = true;  	break;
            case 5: v = new Diferenca( Operadores.DIFERENCA.getOperador() );						this.exigeParametro = false; 	break;
            case 6: v = new Ordenacao( Operadores.ORDENACAO.getOperador() );						this.exigeParametro = true;		break;
            case 7: v = new JuncaoExternaEsquerda( Operadores.JUNCAO_EX_ESQUERDA.getOperador() );	this.exigeParametro = true;		break;
        }
    ...
    public final void alterarParametroOperador(String parm1Operador, String parm2Operador) 
    {
        if(this.painelGrafico.getSelectionCells().length == 1) 
        {
            DefaultGraphCell celula = (DefaultGraphCell)this.painelGrafico.getSelectionCell();
