...
public class Digrafo {
    ...
    /**
     * Apaga todas as arestas que v possui 
     * (pode ser implementado no evendo do botao direito um 'Remove ligacoes') 
     * @param v
     */
    ...
    public final void removeAresta(Vertice v1, Vertice v2) {
        for(int i=0;i<arestas.size();i++){
            if(arestas.get(i).contemVertice(v1) && arestas.get(i).contemVertice(v2)) 
                arestas.remove(i);
        }
    }
    ...
    /**
     * Outra funcao pra c usada no evendo do 
     * botao direito (remove todas as ligacoes)
     */
    ...
    public final String montaAlgebraRelacional( Vertice v , Vertice parent )
    {
    	...
    	try {
            ...
            if( v instanceof Tabela ) 
            {
            	...
            	return "SELECT " + tab.getColumnNames() + " FROM " + v.toString() + " " + tab.getUniqueId();
            }
     ...
     * logica do operador de selecao
     * @param v
     * @param parent
     * @return
     * @throws Exception 
     */
    public final String logicSelecao( Vertice v , Vertice parent ) throws Exception 
    {
        ...
        AliasHelper.getInstance().setCurrentOperator( Operadores.SELECAO.getOperador() );
        ...
        Tabela t1 = null;
        if( op.equals( Operadores.SELECAO.getOperador() ) && !cp.equals( Operadores.SELECAO.getOperador() ) ) {
        ...
        for( Coluna c : t1.getColunas() ) {
        	String fullColumn = String.valueOf( c.getNmTabela() + "." + c.getNmRealColuna() );
        	if( params.indexOf( fullColumn ) != -1 ) {
        		String newFullColumn = String.valueOf( c.getUniqueTable() + "." + c.getNmColuna() );
        ...
        { // Aninhamento de restricoes
            ...
            { // Adiciona antes do group by caso exista
                return parentLeft.split("GROUP BY")[0].trim() + " WHERE " + operParams + " GROUP BY" + parentLeft.split("GROUP BY")[1];
            }
            
            if( parentLeft.contains("ORDER BY")) 
            { // Adiciona antes do group by caso exista
                return parentLeft.split("ORDER BY")[0].trim() + " AND " + operParams + " ORDER BY" + parentLeft.split("ORDER BY")[1];
            }
            ...
            if( parentLeft.contains("GROUP BY")) { // Adiciona antes do group by caso exista
                
            	if( useHaving ) {
            		return parentLeft.split("GROUP BY")[0].trim() + " GROUP BY " + parentLeft.split("GROUP BY")[1] + " HAVING " + operParams;
            	}
            ...
            if( parentLeft.contains("ORDER BY")) 
            { // Adiciona antes do group by caso exista
                return parentLeft.split("ORDER BY")[0].trim() + " WHERE " + operParams + " ORDER BY" + parentLeft.split("ORDER BY")[1];
            }
    ...
    /**
     * Método responsável pela contrução
     * logica do operador de projecao
     * @param v
     * @param parent
     * @return
     * @throws Exception 
     */
    public final String logicProjecao( Vertice v , Vertice parent ) throws Exception 
    {
        ...
        AliasHelper.getInstance().setCurrentOperator( Operadores.PROJECAO.getOperador() );
        ...
        while( st.hasMoreTokens() ) 
        {
    		...
    		if( param.indexOf(".") != -1 ) {
                for( Coluna nT1c : t1.getColunas() ) {
                	...
                	String lastCmd = nT1c.getNmTabela() + "." + nT1c.getNmRealColuna();
                	if( param.equals(lastCmd) ) {
                    	if( !nT1c1.getAliasColuna().trim().isEmpty() ) {
                    		nT1c1.setNmColuna( nT1c.getAliasColuna() );
                    		nT1c1.setAliasColuna( nT1.getUniqueId() + nT1c.getAliasColuna() );
                    	}
            	...
            	for( Coluna cols : t1.getColunas() ) {
            		if( cols.getNmRealColuna().equals(param) ) {
                    	...
                    	if( !c.getAliasColuna().trim().isEmpty() ) {
                    		...
                    		c.setAliasColuna( nT1.getUniqueId() + cols.getAliasColuna() );
                    	}
        ...
        String newQuery = "SELECT " + nT1.getParameters() + " FROM ( " + parentLeft + " ) " + nT1.getUniqueId();
        return makeQuerySafe( newQuery );
    }
    
    /**
     * Método responsável pela contrução
     * logica do operador de ordenacao
     * @param v
     * @param parent
     * @return
     */
    public final String logicOrdenacao( Vertice v , Vertice parent ) throws Exception 
    {
    	...
    	AliasHelper.getInstance().setCurrentOperator( Operadores.ORDENACAO.getOperador() );
     ...
     * @return
     */
    public final String logicJuncaoExEsquerda( Vertice v , Vertice parent ) throws Exception
    {
    	JuncaoExternaEsquerda  jes = (JuncaoExternaEsquerda)v;
    	
    	// Recursivamente chama os operadores superiores, motando suas querys.
        String parentLeft	= montaAlgebraRelacional( getAdjacencias(v).get(0) , v ).trim();
        this.validaQuery(parentLeft);
        Tabela t1 = AliasHelper.getInstance().getLast();
        
        String parentRight	= montaAlgebraRelacional( getAdjacencias(v).get(1) , v ).trim();
        this.validaQuery(parentRight);
        Tabela t2 = AliasHelper.getInstance().getLast(); 

        AliasHelper.getInstance().setCurrentOperator( Operadores.JUNCAO_EX_ESQUERDA.getOperador() );
        ...
        for( Coluna c2 : nT1.getColunas() ) {
        	String fullColumn = String.valueOf( c2.getNmTabela() + "." + c2.getNmRealColuna() );
        	if( onParams.indexOf( fullColumn ) != -1 ) {
        		String newFullColumn = String.valueOf( c2.getUniqueTable() + "." + c2.getNmColuna() );
        		onParams = onParams.replaceAll( fullColumn , newFullColumn );
        	}
        }

        for( Coluna c1 : nT2.getColunas() ) {
        	String fullColumn = String.valueOf( c1.getNmTabela() + "." + c1.getNmRealColuna() );
        	if( onParams.indexOf( fullColumn ) != -1 ) {
        		String newFullColumn = String.valueOf( c1.getUniqueTable() + "." + c1.getNmColuna() );
        		onParams = onParams.replaceAll( fullColumn , newFullColumn );
        	}
        }
    ...
    /**
     * Método responsável pela contrução
     * logica do operador de produto cartesiano
     * @param v
     * @param parent
     * @return
     */
    public final String logicProdutoCartesiano( Vertice v , Vertice parent ) throws Exception
    {
    	// Recursivamente chama os operadores superiores, motando suas querys.
        String parentLeft = montaAlgebraRelacional( getAdjacencias(v).get(0) , v ).trim();
        Tabela t1 = AliasHelper.getInstance().getLast();
        ...
        AliasHelper.getInstance().setCurrentOperator( Operadores.PRODUTO_CARTESIANO.getOperador() );
    ...
    public final String logicUniao( Vertice v , Vertice parent ) throws Exception 
    {
    	...
    	AliasHelper.getInstance().setCurrentOperator( Operadores.UNIAO.getOperador() );
    ...
    /**
     * Método responsável pela contrução
     * logica do operador de agrupamento
     * @param v
     * @param parent
     * @return
     */
    public final String logicAgrupamento( Vertice v , Vertice parent ) throws Exception
    {
        ...
        AliasHelper.getInstance().setCurrentOperator( Operadores.AGRUPAMENTO.getOperador() );
        Tabela nT1 = AliasHelper.getInstance().getNewTableFrom(t1);
        ...
        while( st.hasMoreTokens() ) {
        	...
        	if( param.indexOf("(") != -1 && param.indexOf(")") != 1 ) {
        		...
        		String nmReal = param.split("\\(")[1].trim().split("\\)")[0].trim();
        ...
        if( colAg.trim().isEmpty() ) {
            return "SELECT " + nT1.getParameters() + " FROM ( " + parentLeft + " ) " + nT1.getUniqueId(); //concatena os campos projetados
        }
        // Nao possui colunas projetadas (funcao)
            ...
            if( parentLeft.contains("GROUP BY") ) { 
                return "SELECT " + nT1.getParameters() + " FROM ( " + parentLeft + " ) " + nT1.getUniqueId() + ", " + nT1.getGroupByParameters();
            }
            
            return "SELECT "+ nT1.getParameters() + " FROM ( " + parentLeft + " ) " + nT1.getUniqueId() + " GROUP BY " + nT1.getGroupByParameters();
        }
    ...
    /**
     * Método responsável pela contrução
     * logica do operador de diferenca
     * @param v
     * @param parent
     * @return
     * @throws Exception 
     */
    public final String logicDiferenca( Vertice v , Vertice parent ) throws Exception 
    {
    	//Recupera o SQL resultante dos vertices adjacentes
    	String parentLeft =  montaAlgebraRelacional( getAdjacencias(v).get(0) , v );
    	Tabela t1 = AliasHelper.getInstance().getLast();
    	...
    	AliasHelper.getInstance().setCurrentOperator( Operadores.DIFERENCA.getOperador() );
    	boolean first = false;
    	...
    	{
    		query.append(" AND ");
        	query.append( nT11.getColunas().get(i).getUniqueTable() + "." + nT11.getColunas().get(i).getNmColuna() );
        	query.append(" = ");
        	query.append( nT21.getColunas().get(i).getUniqueTable() + "." + nT21.getColunas().get(i).getNmColuna() );
        	query.append(" ");
    	}
    ...
    /**
     * Altera as clausulas de SELECT, WHERE e GROUP BY
     * das querys para que outros metodos não as
     * altere atraves das verificações da funcao "contains"
     * @param query
     * @return
     */
    private String makeQuerySafe( String query ) {
    	return query.replace("SELECT", "@S#ELECT@")
    				...
    				.replace("GROUP BY", "@G#ROUP BY@")
    				.replace("ORDER BY", "@O#RDER BY@");
    }
    ...
    private void validaQuery( String parentLeft ) throws Exception {
    	if( parentLeft == null || String.valueOf(parentLeft).equals("null") || String.valueOf(parentLeft).trim().isEmpty() ) {
    		throw new Exception();
    	}
    }
}